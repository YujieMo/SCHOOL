...
class embedder:
    def __init__(self, args):
        ...
        if args.dataset == "ACM":
            features, node_rel, edge_dict, labels, node_idx = data_load.load_ACM(args)
        if args.dataset == "Yelp":
            features, node_rel, edge_dict, labels, node_idx = data_load.load_Yelp(args)
        if args.dataset == "DBLP":
            features, node_rel, edge_dict, labels, node_idx = data_load.load_DBLP(args)
        if args.dataset == "Aminer":
            features, node_rel, edge_dict, labels, self.train_idx, self.val_idx, self.test_idx, node_idx = data_load.load_Aminer(args)
        elif args.dataset in ['computers', 'photo','cs', 'physics', 'cora', 'citeseer', 'pubmed']:
