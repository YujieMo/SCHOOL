...
def load_ACM(args):
    ...
    return features, node_rel, edge_dict, labels, node_idx
...
def load_Yelp(args):
    ...
    return features, node_rel, edges, labels, node_idx
...
def load_IMDB(args):
    ...
    edge_dict['m-d'].append(edges[0])
    edge_dict['d-m'].append(edges[1])
    edge_dict['m-a'].append(edges[2])
    edge_dict['a-m'].append(edges[3])
    node_rel = defaultdict(list)

    node_rel['m'].append('m-d')
    node_rel['m'].append('m-a')
    node_rel['d'].append('d-m')
    node_rel['a'].append('a-m')
    node_idx = {}
    node_idx['m'] = torch.LongTensor([i for i in range(4661)])
    node_idx['d'] = torch.LongTensor([i for i in range(4661, 6931)])
    node_idx['a'] = torch.LongTensor([i for i in range(6931, 12772)])
...
def load_Aminer(args):
    ...
    test_idx = test[0]
    node_idx = idx
    return features, node_rel, edge_dict, labels, train_idx, val_idx, test_idx, node_idx
